...
public class AnimatorEventLayer {
	...
	public void MakeDictionaries () {
		...
		{
			if(! transitionNames.ContainsKey(_transitionKeys[i]) ) ////remove duplicate content : multiple animation
			{
				...
				List<int> transkeylist= new List<int>(_transitionKeys);
				transkeylist.RemoveAt(i);
				_transitionKeys = transkeylist.ToArray();
			}*/
		}
